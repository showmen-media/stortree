...
0. Repo skeleton: `ansible.cfg`, sample `inventory/hosts.yml`, empty role
   skeletons (`tasks/main.yml`, `meta/main.yml`) for every role listed
   under "Config layout" above, `filter_plugins/stortree.py` stubbed out,
   and the Molecule scaffolding for a `default` scenario. Docs (this
   plan, the schema reference) are phase 0's actual output before any
   role has real tasks in it.
1. `resolve()` (§1) — pure functions in `filter_plugins/stortree.py`,
   ...
   invoked. `stortree_facts` wraps it into `set_fact`.
2. `stortree_common` (service account, `/srv/stortree`, `/etc/stortree`)
   and `stortree_mounts` (§2), applied to a single host in the Molecule
   harness. Verify against a real remote outside the harness too.
3. `stortree_samba` (§4) from resolved server subtrees, using plain Unix
   permissions (no ACLs yet).
4. `stortree_identity` (§5) — POSIX attribute exposure is a requirement,
   not something this phase verifies (see §5).
5. `stortree_acl` (§6) wired to resolved `access` blocks + SSSD groups.
6. `stortree_pam_smbpass` (§5), plus `stortree_sshd` (§6, only runs when
   the optional file is present).
7. `stortree_secrets` (§3): filtered per-host `rclone.conf` rendering,
   vault integration for the master files.
8. `stortree_peer_trust` (§7): keypair provisioning + cross-host
   `authorized_keys`, exercised in the Molecule `full-tree` scenario with
   a genuine two-host peer dependency.
9. `playbooks/status.yml`, the `full-tree` Molecule scenario tying every
   role together end to end, and a runbook doc covering the operator
   commands in §8.

Each phase produces something usable/testable on its own before the next
starts.
